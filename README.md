...
| **Médias et documents — repli à quatre paliers, audio et vidéo (§38)** | `lib/media/paliers.ts` (pur : `palierRequis` (NATIF → OCR → VISION_RAPIDE → VISION_SUPERIEURE, par page), `planifier` sous `BUDGETS` par exigence (`rapide` / `auto` / `precis`) et `PLAFOND_SUPERIEUR_ABSOLU` = 8 — jamais 500 pages dans un gros modèle —, pages visées devant, `rapport` par méthode + hors budget, `confianceDe` (VÉRIFIÉ / PROBABLE / INCERTAIN), `COUT_PAGE_USD`), `lib/media/transcription.ts` (pur : `normaliserSegments`, `formatHorodatage`, `chercher` (« où exactement X a-t-il parlé de Y » → index, instant, locuteur, extrait, filtre par locuteur), `decouperEnChapitres` (silences, durée, changement de vocabulaire), `attribuerLocuteurs`, `locuteursDe`, `texteHorodate`, `instantsAregarder`), `lib/media/stt.ts` (`transcrireAvecSegments` : `verbose_json` + segments, langue, durée, indice de vocabulaire, `fetch` injectable, 25 Mo dits, vidéo acceptée telle quelle), `platform/in-process/media/lecture.ts` (`lireDocumentParPaliers` : MuPDF → `ocrDocument` → `callLuna` sur pages rastérisées → `askModelJsonAvecImages` (passerelle, orchestrateur) ; trois tours replanifiés sur ce qui reste du budget ; bilan, coût réel), `platform/in-process/media/transcription.ts` (`transcrireMediaDrive` : STT → diarisation par fenêtres (Luna, participants connus, sinon Locuteur A/B, PROBABLE) → chapitres → extraction (décisions, engagements, actions, entités, questions, chacun avec son instant) → `MediaTranscript` (une par version) → `indexDriveNodeText` ; `chercherDansMediaDrive`, `regarderVideoDrive` (instants pertinents, six images au plus, ffmpeg s'il existe — sinon la limite est dite), `resoudreMedia`), `lib/models/contract.ts` + `openai.ts` + `openai-responses.ts` + `anthropic.ts` + `gateway.ts` (bloc `image` base64 dans un tour utilisateur ; `askModelJsonAvecImages`, `IMAGES_MAX_PAR_APPEL` = 8 ; une image pèse ~1 000 jetons dans l'estimation, jamais sa longueur), `assistant/media-tools.ts` (`media_transcript` : transcrire / chercher / structure / regarder), `assistant/office-capabilities.ts` (`pdf_read` lit par paliers : `niveau`, `question`, méthode et confiance par page, hors budget dit), `assistant-files.ts` (une pièce jointe audio / vidéo est transcrite, horodatée, dite PROBABLE), `prisma` `MediaTranscript` (+ migration `20261027090000_media_transcripts`). Banc : `paliers.test.ts` (6, mesure `paliers_plafond`), `transcription.test.ts` (5, mesure `media_instant_exact`), `stt.test.ts` (3), `platform/in-process/media/media.test.ts` (paliers sur PDF réel, transcription persistée et cherchée, vidéo sans ffmpeg, outil), défi live `defi-media-reunion` (voix de synthèse → Drive → « où parle-t-on du budget ? » → l'instant) |
...
| **Legal — engagements & échéances** | `lib/legal/lifecycle.ts` (pur : `expiryLevel`, `shouldRemind`, `expiryMessage`, `proposeRenewalDates` + tests) ; `lib/legal/expiry-sweep.ts` (`runLegalExpirySweep`, branché dans `lib/scheduled.ts` — aligne le statut échu, prévient à l'entrée d'une zone d'urgence, verrou atomique sur `lastRemindedAt`) ; `app/(app)/legal/` (`page.tsx`, `legal-table.tsx`, `legal-fields.ts`, `[id]/`). `EntityType.LEGAL_DOCUMENT`. **Les FACTURES y vivent** (nature `INVOICE`) : `lib/legal/invoices.ts` (pur : `invoiceTally`, `legalViewScope`, `legalWriteAllowed`, `natureFromParam`), `lib/labels.ts` (`settlementState`, `INVOICE_SETTLEMENT` — l'état de règlement DÉDUIT, en trois valeurs), `lib/finance/settle-invoice.ts` (l'écriture au livre), `lib/actions/invoice-actions.ts` (le vocabulaire « facture », adaptateur du registre). `/legal?nature=INVOICE` est une VUE, pas un écran. |
...
| **Liaisons transverses** | `lib/links/source-link.ts` (pur : `LINKABLE_SOURCES`, `sourceHref`, `sourceCaption` — un test remonte CHAQUE route jusqu'à `NAVIGATION` pour interdire les liens morts) ; `components/shared/linked-records.tsx` (bloc serveur posable sur toute fiche) + `attach-to-source.tsx` (créer une pièce déjà rattachée). Les modèles `LegalDocument` (factures comprises : nature `INVOICE`) et `MailEntry` portent `sourceType`/`sourceId`. |
