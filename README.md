...
| **Voix — barge-in confirmé & VAD réglable** | Module PUR `lib/assistant/voice-tuning.ts` (`bargeInDecision` — mots transcrits / parole soutenue / bruit bref —, `isNoiseTranscript`, `buildTurnDetection` env-réglable, seuils `BARGE_IN_SUSTAIN_MS`/`BARGE_IN_NOISE_MS`) ; provider `app/(app)/assistant/realtime-voice.ts` (fenêtre d'évaluation sur `speech_started`, confirmation → `response.cancel` + `output_audio_buffer.clear` + `conversation.item.truncate` via `response.output_item.added`, filtre bruit sur la transcription, métriques possible/confirmé/ignoré) ; `interrupt_response:false` par défaut dans la session (`voice-realtime.ts` → `buildTurnDetection`) ; journal `voice_false_barge_in_ignored` / `voice_barge_in_confirmed` (`call-provider.tsx` + `/api/assistant/voice/log`). Tests : `voice-tuning.test.ts`. |
