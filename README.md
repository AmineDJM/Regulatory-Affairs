...
| **Voix — barge-in confirmé & VAD réglable** | Module PUR `lib/assistant/voice-tuning.ts` (`bargeInDecision` — mots transcrits / parole soutenue / bruit bref / **auto-protection écho** (`audioPlaying` : haut-parleur actif → seuls des mots confirment) —, `isNoiseTranscript`, `buildTurnDetection` env-réglable, seuils `BARGE_IN_SUSTAIN_MS`/`BARGE_IN_NOISE_MS`) ; provider `app/(app)/assistant/realtime-voice.ts` (fenêtre d'évaluation liée au SEGMENT `item_id`, confirmation → `response.cancel` + `output_audio_buffer.clear` + `conversation.item.truncate`, confirmation TARDIVE sur transcription lente, debounce un segment = une coupure, filtre bruit + `conversation.item.delete` du bruit committé + annulation de la réponse fantôme) ; `interrupt_response:false` par défaut dans la session (`voice-realtime.ts` → `buildTurnDetection`) ; journal `voice_false_barge_in_ignored` / `voice_barge_in_confirmed` / `voice_phantom_response_cancelled` (`call-provider.tsx` + `/api/assistant/voice/log`). Tests : `voice-tuning.test.ts`, `voice-pipeline.test.ts`. |
