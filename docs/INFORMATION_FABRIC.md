...
| **F8 — Provenance au niveau du fait** | `fabric/provenance.ts` : vocabulaire `FaitSource` (source ERP / document / e-mail / fil / page PDF / cellule / pièce / réunion / personne / externe / calcul, date propre, instant de lecture, confiance + fondement, fraîcheur, autorité et preuve négative DU REGISTRE F3, outil = droits), extraction déterministe des sorties d'outils (liens internes seulement ; l'externe est cité, jamais suivi), faits déclarés `_provenance` revalidés, `faitCalcule` (entrées + transformation + formule + date, pire confiance héritée), `repondreProvenance` (ancres numériques / nominales ; sans ancre, le DERNIER tour même vide). `fabric/provenance-store.ts` + table `AssistantProvenance` (migration `20261022090000`) : une ligne par tour, cloisonnée par personne, relecture indexée (≤ 6 tours), rétention 30 jours. Câblage : `lectures` + `avecProvenance` dans les deux boucles, consignation par les trois entrées (flux, action, voix), forme `PROVENANCE` du routeur vocal → réponse AVANT tout modèle, panneau Sources avec `detail`, `finance_totals._provenance`, `source_map` dans le domaine `SOURCES`. Mesures : P95 relecture 5 ms (40 lectures, registre de 40 tours × 40 faits, local), tour complet dans le navigateur < 5 s (e2e), suite live 0 appel de modèle. | **TESTED** (16 + 4 tests fabric, routeurs, e2e, live, défi) |
| **F9 — Résolution d'entités** | `fabric/entites-score.ts` (scoreur pur : épreuves identique / sans générique / ordre / acronyme / jetons / typo, verdicts avec écart net, question de désambiguïsation, preuves structurelles vs ressemblance) + `fabric/entites.ts` (dix natures, identifiant exact → contenu → trigramme `pg_trgm`/`unaccent`, alias produits, molécules triées, aucune écriture). Câblage : `resoudreMentions` sur les entités du plan dans les deux boucles (bloc ENTITÉS RÉSOLUES), `resolvePerson` par la brique. Banc : 60 mentions réalistes → 60/60, P50 3 ms, P95 12 ms. | **TESTED** (5 purs + 3 sur base) |
