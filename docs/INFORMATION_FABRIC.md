...
| **F3 — Source Registry** | Registre central typé des familles de sources : modes de recherche, autorité, fraîcheur, preuve négative, coût. Consommé par recours mission + `source_map`. | GAP |
| **F4 — Entités & liens** | Extraction des mentions (produits/personnes/orgs canoniques) à l'ingestion → table indexée → « tout ce qui est relié à X » instantané. | GAP |
| **F5 — Hot data & précalcul** | États 360 persistés + fraîcheur affichable + invalidation par événement/battement. | GAP |
| **F6 — Bulk & missions** | Primitive multi-get (N ids → K appels physiques), branchée dans l'éventail ; mesure logique vs physique. | GAP |
