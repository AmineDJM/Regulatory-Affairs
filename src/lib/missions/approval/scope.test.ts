...
const CONNUES = [
  "directory_list", "employee_360", "inspect_record", "prepare_mail",
  "send_prepared_mail", "send_erp_message", "notify_person",
];
...
describe("§32 — un seul accord pour toute une mission", () => {
  ...
  it("seules les étapes qui en ont besoin entrent dans le périmètre", () => {
    const p = perimetre(compiler([
      ...
      { key: "envoi", title: "Envoi", capability: "send_prepared_mail", input: { to: "a@x.dz" } },
    ]))!;
  ...
  it("le niveau vient de la PIRE étape, pas de la plus courante", () => {
    const steps: PlannedStep[] = [
      ...Array.from({ length: 20 }, (_, i) => ({
        key: `m${i}`, title: `M${i}`, capability: "send_erp_message", input: { to: `p${i}` },
      })),
      { key: "mail", title: "Mail", capability: "send_prepared_mail", input: { to: "x@y.dz" } },
    ];
...
describe("§33 — l'empreinte du périmètre", () => {
  const base: PlannedStep[] = [
    { key: "envoi", title: "Envoi", capability: "send_prepared_mail", input: { to: "alla@x.dz", corps: "Bonjour" } },
  ];
  ...
  it("ne bouge PAS quand l'ordre des clés de l'entrée change", () => {
    const inverse: PlannedStep[] = [
      { key: "envoi", title: "Envoi", capability: "send_prepared_mail", input: { corps: "Bonjour", to: "alla@x.dz" } },
    ];
  ...
  it("ne bouge PAS quand l'ordre des ÉTAPES change", () => {
    const a: PlannedStep[] = [
      { key: "e1", title: "E1", capability: "send_erp_message", input: { to: "p1" } },
      { key: "e2", title: "E2", capability: "send_erp_message", input: { to: "p2" } },
    ];
  ...
  it("ne bouge pas quand un champ matériel IMBRIQUÉ est reconstruit dans un autre ordre", () => {
    ...
    const a = compiler([{
      key: "p", title: "P", capability: "send_erp_message",
  ...
  it("BOUGE quand un montant change", () => {
    const a = perimetre(compiler([
      { key: "p", title: "P", capability: "send_erp_message", input: { to: "compta", montant: 100000 } },
    ]))!;
    const b = perimetre(compiler([
      { key: "p", title: "P", capability: "send_erp_message", input: { to: "compta", montant: 900000 } },
    ]))!;
    expect(a.scopeHash).not.toBe(b.scopeHash);
  });

  it("BOUGE quand une action externe est AJOUTÉE", () => {
    const plus = [...base, { key: "b", title: "B", capability: "send_erp_message", input: { to: "x" } }];
  ...
  it("BOUGE quand la source d'un éventail change : ce n'est plus la même liste de gens", () => {
    const a = compiler([
      ...
      { key: "m", title: "M", capability: "send_erp_message", forEach: { from: "l", path: "employes", as: "e" }, input: { to: "{{e.id}}" } },
    ]);
    const b = compiler([
      { key: "l", title: "L", capability: "directory_list" },
      { key: "m", title: "M", capability: "send_erp_message", forEach: { from: "l", path: "prestataires", as: "e" }, input: { to: "{{e.id}}" } },
    ]);
...
describe("§33 — un changement ne réouvre QUE la partie modifiée", () => {
  ...
  it("une étape AJOUTÉE est la seule à redemander son accord", () => {
    ...
    const augmente = compiler([...initial, { key: "e3", title: "E3", capability: "send_erp_message", input: { to: "p3" } }]);
...
describe("l'empreinte d'une SEULE étape — pour dire laquelle a changé", () => {
  it("distingue deux étapes différentes et reconnaît la même", () => {
    const m = compiler([
      { key: "a", title: "A", capability: "send_erp_message", input: { to: "p1" } },
      { key: "b", title: "B", capability: "send_erp_message", input: { to: "p2" } },
    ]);
    const [a, b] = m.steps;
    expect(empreinteEtape(a)).not.toBe(empreinteEtape(b));
    expect(empreinteEtape(a)).toBe(empreinteEtape(compiler([
      { key: "a", title: "titre tout autre", capability: "send_erp_message", input: { to: "p1" } },
    ]).steps[0]));
  });
});
