...
const CAS: Cas[] = [
  ...
  // ── DANS LA FOULÉE : la personne est encore dans la conversation, qui vient de le lui dire ──
  { nom: "surveillance posée en 5 s (effet interne, aucun livrable)", signal: { ...base, kind: "MISSION_COMPLETED", bilan: { ...bilan(1, 1, ["watch_entity"]), effetsExternes: false }, dureeMs: 5_000 }, attendu: "JOURNAL", pourquoi: "« préviens-moi seulement s'il y a un problème » : la conversation a confirmé, le téléphone n'a pas à vibrer" },
  { nom: "rappel posé en 8 s (effet interne)", signal: { ...base, kind: "MISSION_COMPLETED", bilan: { ...bilan(1, 1, ["plan_reminder"]), effetsExternes: false }, dureeMs: 8_000 }, attendu: "JOURNAL", pourquoi: "même politique que la conversation, qui pose un rappel sans notifier" },
  { nom: "e-mail parti en 40 s (effet externe)", signal: { ...base, kind: "MISSION_COMPLETED", bilan: { ...bilan(2, 2, ["send_email"]), effetsExternes: true }, dureeMs: 40_000 }, attendu: "INFO", pourquoi: "quelque chose a quitté la maison : le dirigeant doit le savoir" },
  { nom: "export livré en 90 s (livrable)", signal: { ...base, kind: "MISSION_COMPLETED", bilan: { ...bilan(3, 3, [], ["Suivi regulatory.xlsx"]), effetsExternes: false }, dureeMs: 90_000 }, attendu: "INFO", pourquoi: "un fichier l'attend, quelle que soit la vitesse" },
  { nom: "tâche créée puis mission finie trois jours plus tard", signal: { ...base, kind: "MISSION_COMPLETED", bilan: { ...bilan(4, 4, ["create_task"]), effetsExternes: false }, dureeMs: 3 * 24 * 3600 * 1000 }, attendu: "INFO", pourquoi: "la conversation est loin : la fin mérite une information" },
