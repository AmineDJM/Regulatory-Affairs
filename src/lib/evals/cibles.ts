...
export const CIBLES: readonly Cible[] = [
  ...
  { id: "chaos_tenu", libelle: "les situations adverses sont tenues : fournisseur qui tombe, crash, redéploiement, règle changée en cours, événements dans le DÉSORDRE, échéance déjà passée, dizaines de missions simultanées — rien perdu, rien dupliqué, rien inventé, pas de spam", mandat: "§17 / §33 — 100 %", cible: 1, sens: "min", unite: "taux", mesure: "platform/in-process/evals/sabotages.test.ts" },
  { id: "composition_libre_rendu_ferme", libelle: "la composition d'écran est libre (six contenants, profondeur quelconque) mais aucune chaîne issue d'un modèle ne devient du balisage : les feuilles DÉSIGNENT des blocs existants", mandat: "Mandat 7 — 100 %", cible: 1, sens: "min", unite: "taux", mesure: "lib/planche/planche.test.ts" },
  { id: "agencement_refuse_garde_le_fond", libelle: "un agencement refusé retombe sur une pile et le DIT : perdre la mise en page est une gêne, perdre le résultat est une panne", mandat: "Mandat 7 — 100 %", cible: 1, sens: "min", unite: "taux", mesure: "lib/planche/planche.test.ts" },
  { id: "angle_dit_ce_qu_il_ecarte", libelle: "un angle ne relit rien et annonce toujours combien de lignes il a écartées et pourquoi — « 28 sur 34 » sans le dire ment par omission", mandat: "Mandat 7 — 100 %", cible: 1, sens: "min", unite: "taux", mesure: "lib/planche/planche.test.ts" },
  { id: "blocs_rendus_sans_derive", libelle: "la liste des blocs acceptés à la composition colle exactement à ce que le composant sait rendre — un écart afficherait un trou là où le compilateur a dit oui", mandat: "Mandat 7 — 100 %", cible: 1, sens: "min", unite: "taux", mesure: "lib/planche/planche.test.ts" },
  ...
  { id: "plan_couvre_objectif", libelle: "plans jugés correctement sur la COUVERTURE des primitives que la demande exige — un plan « lire puis répondre » pour une demande chiffrée était accepté, compilé et exécuté, l'absence n'étant constatée qu'au contrôle final, après que tout avait tourné et coûté", mandat: "§118 — 100 %", cible: 1, sens: "min", unite: "taux", mesure: "lib/missions/compiler/couverture.test.ts" },
  { id: "lecture_visible_des_quelle_rend", libelle: "sorties de lecture rendues VISIBLES à l'instant où elles rendent, au lieu d'attendre la rédaction du modèle — chronologie mesurée sur l'analyse Regulatory : onze pièces nommées et liées connues à 108 ms, premier bloc à l'écran à 13 742 ms", mandat: "§118 — TTUV", cible: 1, sens: "min", unite: "taux", mesure: "lib/assistant/workspace/apercu.test.ts" },
  { id: "sortie_reelle_impossible_en_test", libelle: "modules capables d'atteindre une personne réelle (courriel, notification, agenda) qui passent par la PORTE DE SORTIE — elle refuse à l'exécution en mode test, et un second test refuse à la relecture qu'un canal soit branché sans elle : une convention est une prière, un refus de compilation est une propriété", mandat: "§118 — 0 effet externe", cible: 1, sens: "min", unite: "taux", mesure: "lib/sortie/garde.test.ts" },
  { id: "sortie_bloquee_par_la_porte", libelle: "tentatives de sortie interceptées AVANT l'ouverture du transport, en appelant les VRAIES fonctions de production (`sendMail` SMTP, `sendPushToUser` web-push, le fournisseur Graph) — le test distingue un refus de la garde d'une simple erreur réseau, sans quoi « ça a échoué » passerait pour une preuve de sécurité", mandat: "§118 — 0 effet externe", cible: 1, sens: "min", unite: "taux", mesure: "lib/sortie/transports.test.ts" },
  { id: "cible_ambigue_rend_des_candidats", libelle: "écritures jugées correctement sur la DÉSIGNATION de leur cible — mesuré au banc : « Corrige ça » après une liste de documents rendait « Je propose : ANNULER le devis DEV-2026-0038 », cible choisie sans raison et verbe transformé ; le score n'a de sens qu'avec ses DEUX jeux, car une porte qui refuse tout serait parfaite sur l'un et nulle sur l'autre", mandat: "§104.7 étendu — 0 faux succès", cible: 1, sens: "min", unite: "taux", mesure: "lib/assistant/cible-designee.test.ts" },
  { id: "un_seul_chemin_vers_les_modeles", libelle: "fichiers de src/ qui n'ouvrent PAS leur propre connexion a un fournisseur de modele - mesure apres avoir trouve, au banc live, un second passage (`lib/ai.ts`) cable en dur sur Anthropic, emprunte par 27 appelants dont la memoire durable d'Adam, et dont AUCUN jeton n'etait compte", mandat: "§50 - observabilite et attribution du cout", cible: 1, sens: "min", unite: "taux", mesure: "lib/models/passerelle-unique.test.ts" },
  { id: "proposition_depassee_retiree", libelle: "cartes d'ecriture jugees correctement sur leur PEREMPTION — le banc live a joue « une tache pour Raihana » puis « non, finalement Amel, et lundi » : les DEUX cartes restaient executables, et le clic sur la premiere a cree la tache que le PDG venait d'annuler ; le score n'a de sens qu'avec ses DEUX jeux, car une regle qui retire tout serait parfaite sur l'un et nulle sur l'autre", mandat: "§11 — la contrainte que la personne vient d'enoncer ne se coupe jamais", cible: 1, sens: "min", unite: "taux", mesure: "lib/assistant/proposition-perimee.test.ts" },
  { id: "primitive_exigee_deduite", libelle: "demandes INÉDITES (jeu tenu à l'écart, écrit après le dictionnaire et jamais relu pour l'ajuster) dont les primitives exigées sont déduites exactement — ni manquée ni inventée : une exigence manquée laisse passer un plan qui ne répond pas, une exigence inventée enferme une mission correcte", mandat: "§118 — généralisation", cible: 0.9, sens: "min", unite: "taux", mesure: "lib/missions/planner/primitives.test.ts" },
  { id: "primitive_faux_positifs", libelle: "questions de LECTURE écrites pour un AUTRE banc (corpus de connaissance) qui ne déclenchent aucune exigence à tort — c'est le seul jeu que personne n'a pu ajuster, donc le seul juge honnête du sur-déclenchement : une exigence inventée fait refuser un plan correct et enferme la mission", mandat: "§118 — une hésitation n'enferme jamais", cible: 0.8, sens: "min", unite: "taux", mesure: "lib/missions/planner/primitives.test.ts" },
  { id: "motif_echec_avec_recours", libelle: "part des motifs d'échec RÉELLEMENT émis par le runtime qui ont une échelle de recours — `tenterRecours` rejette d'entrée tout motif absent de la taxinomie, donc un motif oublié désarme silencieusement la persévérance (§9) pour toute une famille d'échecs", mandat: "§118 — 100 %", cible: 1, sens: "min", unite: "taux", mesure: "lib/missions/runtime/replan.test.ts" },
  { id: "mission_replanifiable_atteinte", libelle: "part des missions en état replanifiable que la requête du battement sélectionne réellement — une mission dont toutes les étapes ont abouti et que le juge refuse n'en portait aucune de PENDING ni FAILED, donc le battement ne la voyait pas et la replanification n'était jamais appelée pour elle", mandat: "§118 — 100 %", cible: 1, sens: "min", unite: "taux", mesure: "platform/in-process/missions/battement-replan.test.ts" },
  { id: "capacite_forme_connue", libelle: "nombre de capacités dont la FORME de sortie est apprise des exécutions réelles — la table écrite à la main en couvrait six sur deux cent vingt-neuf, et pour les autres le planificateur devinait les champs de « {{etape.champ}} »", mandat: "§118 — apprentissage", cible: 6, sens: "min", unite: "nombre", mesure: "platform/in-process/missions/formes.test.ts" },
  { id: "reference_morte_avant_execution", libelle: "une référence « {{etape.champ}} » vers un champ que l'étape amont ne rendra PAS est refusée à la COMPILATION — le moteur la détectait déjà, mais à l'exécution, après l'accord du dirigeant, en tuant la mission sans rejeu", mandat: "§118 — 100 %", cible: 1, sens: "min", unite: "taux", mesure: "lib/missions/compiler/references-mortes.test.ts" },
  { id: "reference_licite_jamais_refusee", libelle: "une référence correcte n'est JAMAIS refusée — surtout vers une capacité jamais observée : un refus sur une ignorance frapperait d'abord les capacités neuves, et échangerait un défaut mesuré contre un défaut invisible", mandat: "§118 — 100 %", cible: 1, sens: "min", unite: "taux", mesure: "lib/missions/compiler/references-mortes.test.ts" },
  { id: "forme_sortie_apprise", libelle: "la FORME de sortie d'une capacité s'apprend de ce qu'elle a réellement rendu — sans elle le planificateur devine les champs de « {{etape.champ}} », et une référence devinée tue la mission à l'exécution", mandat: "§118 — 100 %", cible: 1, sens: "min", unite: "taux", mesure: "lib/missions/registry/formes.test.ts" },
  { id: "capacite_montree_au_planificateur", libelle: "une demande écrite dans les mots d'une personne (« significatif ou du bruit ? », « les mois anormaux », « rédige une note ») fait REMONTER la capacité qui y répond — ce qu'on ne montre pas au planificateur ne peut pas être planifié, et le plan est alors accusé à tort", mandat: "§43 — 100 %", cible: 1, sens: "min", unite: "taux", mesure: "lib/missions/registry/resolve.test.ts" },
  { id: "capacite_jamais_absente_a_tort", libelle: "une capacité que la recherche classe PREMIÈRE n'est jamais déclarée « inexistante » par la détection de manque — les deux réponses du même outil à la même phrase ne se contredisent pas, et Adam croit la seconde", mandat: "§44 — 100 %", cible: 1, sens: "min", unite: "taux", mesure: "lib/registre/registre.test.ts" },
  ...
  { id: "ecartee_pertinente_en_tete", libelle: "l'exclusion qui répond à la demande sort EN TÊTE des écartées : affichée en vingtième derrière du bruit de catalogue, la distinction droit/absence n'atteint jamais le modèle et « je ne peux pas » revient", mandat: "§44 — 100 %", cible: 1, sens: "min", unite: "taux", mesure: "lib/registre/registre.test.ts" },
