...
export default async function CongressInternationalPage() {
  ...
  const [rows, form] = await Promise.all([
    getCongressList("INTL", user),
    canCreate ? getAdProCreateData(user.id, ["CONGRESS_INTERNATIONAL"]) : Promise.resolve(null),
  ]);
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="Prises en charge Internationales" description="Prise en charge de participants à un événement à l'étranger — congrès, séminaire, formation. Validation préliminaire (National Sales), arbitrage budgétaire de Direction Marketing, décision de la Direction.">
        {canCreate && form && (
          <CongressRequestButton doctors={form.doctors} users={form.users}
