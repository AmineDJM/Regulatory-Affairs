...
async function main() {
  ...
  for (const m of MESSAGES) {
    ...
    const { resume, outils, appels } = await withTurn("text", async (trace) => {
      ...
      return { resume: summarize(trace), outils: trace.tools.map((t) => t.name), appels: trace.calls.map((c) => ({ m: c.model, e: c.inputTokens, cache: c.cachedInputTokens, s: c.outputTokens, cout: c.costUsd })) };
    });
